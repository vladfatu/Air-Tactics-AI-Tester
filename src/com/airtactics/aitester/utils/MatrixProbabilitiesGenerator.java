			...
			for (int j = 0; j < 10; j++)
			{
				...
				if (tileMatrix[i][j] == TileType.NONE)
				{
					...
					if (isPlaneValid(head, 0, tileMatrix))
					{
						...
						probabilityMatrix[i][j] += getPlaneBodyHits(head, 0, tileMatrix) * 2;
					}
					if (isPlaneValid(head, 90, tileMatrix))
					{
						probabilityMatrix[i][j]++;
						probabilityMatrix[i][j] += getPlaneBodyHits(head, 90, tileMatrix) * 2;
					}
					if (isPlaneValid(head, 180, tileMatrix))
					{
						probabilityMatrix[i][j]++;
						probabilityMatrix[i][j] += getPlaneBodyHits(head, 180, tileMatrix) * 2;
					}
					if (isPlaneValid(head, 270, tileMatrix))
					{
						probabilityMatrix[i][j]++;
						probabilityMatrix[i][j] += getPlaneBodyHits(head, 270, tileMatrix) * 2;
					}
				}
